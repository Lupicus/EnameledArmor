...
public class EAItemGenerators
{
	private final ItemModelGenerators gen;

	public EAItemGenerators(ItemModelGenerators gen)
	{
		this.gen = gen;
	}
	...
	public void run()
	{
		...
		gen.generateFlatItem(ModItems.EA_DIAMOND_SWORD, ModelTemplates.FLAT_HANDHELD_ITEM);
		gen.generateFlatItem(ModItems.EA_IRON_SWORD, ModelTemplates.FLAT_HANDHELD_ITEM);
		gen.generateFlatItem(ModItems.EA_NETHERITE_SWORD, ModelTemplates.FLAT_HANDHELD_ITEM);
	...
	protected void generateTrimmableItem(Item item)
	{
		...
		if (equippable != null && equippable.slot().getType() == EquipmentSlot.Type.HUMANOID_ARMOR
			...
			for (TrimMaterialData itemmodelgenerators$trimmaterialdata : ItemModelGenerators.TRIM_MATERIAL_MODELS) {
				MaterialAssetGroup assets = itemmodelgenerators$trimmaterialdata.assets();
				ResourceLocation resourcelocation3 = resourcelocation
						.withSuffix("_" + assets.base().suffix() + "_trim");
				ResourceLocation resourcelocation4 = ResourceLocation.withDefaultNamespace(
						"trims/items/" + name + "_trim_" + assets.overrides().getOrDefault(assetId, assets.base()).suffix());
				gen.generateLayeredItem(resourcelocation3, resourcelocation1, resourcelocation2, resourcelocation4);
			...
			ModelTemplates.TWO_LAYERED_ITEM.create(resourcelocation,
					TextureMapping.layered(resourcelocation1, resourcelocation2), gen.modelOutput);
	...
	protected void generateBow2(Item item)
	{
		...
		ItemModel.Unbaked itemmodel$unbaked1 = ItemModelUtils.plainModel(gen.createFlatItemModel(item, "_pulling_0", tmpl));
		ItemModel.Unbaked itemmodel$unbaked2 = ItemModelUtils.plainModel(gen.createFlatItemModel(item, "_pulling_1", tmpl));
		ItemModel.Unbaked itemmodel$unbaked3 = ItemModelUtils.plainModel(gen.createFlatItemModel(item, "_pulling_2", tmpl));
		gen.itemModelOutput.accept(item,
