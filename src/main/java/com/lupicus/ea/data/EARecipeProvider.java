...
public class EARecipeProvider extends RecipeProvider
{
	...
	protected void armorSet(RecipeOutput consumer, ItemLike result, ItemLike input)
	{
		...
		eaShapeless(RecipeCategory.COMBAT, result).setOperation("set_color").requires(result).requires(Tags.Items.MUSHROOMS).requires(TRUE_DYES).addSuffix("_set").save(consumer);
